...
export default function ObservationPanel({
  ...
  const home = world.teamBases[fixture.homeTeamId];
  const away = world.teamBases[fixture.awayTeamId];
  const advanceButton = advanceAction ? (
    <button
      type="button"
      ...
      <div className={advanceButton ? 'grid grid-cols-[minmax(0,0.82fr)_minmax(8.5rem,1.18fr)] gap-2' : undefined}>
        <button
          ...
          aria-label={pending
            ...
            : '做出本轮观察判断 · 可选'}
          className={`flex min-h-11 w-full items-center justify-center gap-2 px-3 text-xs text-sky-300 transition-colors ${
            embedded
              ? 'rounded border border-slate-700/70 bg-slate-900/50 hover:border-sky-800 hover:bg-slate-900/70'
              : 'rounded-lg border border-dashed border-sky-800/50 bg-slate-800/70 hover:border-sky-700 hover:bg-slate-800'
          }`}
        >
          <Icon name={pending ? 'check' : 'target'} size={15} />
          <span className="min-w-0">
            {pending
              ? `已判断：${observationSelectionLabel(pending.selection)}`
              : '本轮判断 · 可选'}
          </span>
        </button>
        {advanceButton}
  ...
  return (
    <>
      <section
        ...
        className={embedded
          ? 'border-y border-sky-900/50 bg-slate-900/35 px-1 py-3'
          : 'rounded-lg border border-sky-800/50 bg-slate-800 p-3'}
      >
