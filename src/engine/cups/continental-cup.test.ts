...
const TEAMS_16 = Array.from({ length: 16 }, (_, i) => `m${i + 1}`);
const TEAMS_8  = Array.from({ length: 8 },  (_, i) => `s${i + 1}`);

describe('initContinentalCup', () => {
  it('rejects mismatched team counts per region', () => {
    expect(() => initContinentalCup('大陆', ['a', 'b'], 1, new SeededRNG(0))).toThrow(/16/);
    expect(() => initContinentalCup('南洲', TEAMS_16, 1, new SeededRNG(0))).toThrow(/8/);
    expect(() => initContinentalCup('东洲', ['a', 'b', 'c'], 1, new SeededRNG(0))).toThrow(/8/);
  });

  it('builds a 大陆杯 R16 round covering all 16 teams', () => {
    const cup = initContinentalCup('大陆', TEAMS_16, 17, new SeededRNG(11));
    ...
    expect(cup.rounds[0].roundName).toBe('R16');
    expect(cup.rounds[0].fixtures).toHaveLength(8);
    // Every team should appear exactly once
    const teamsUsed = cup.rounds[0].fixtures.flatMap((f) => [f.homeTeamId, f.awayTeamId]);
    expect(new Set(teamsUsed).size).toBe(16);
    // Fixture ids embed the season + type
    expect(cup.rounds[0].fixtures[0].id).toMatch(/^CC-mainland_cup-S17-R16-M1$/);
  });

  it('builds a 南洲杯 QF round covering all 8 teams (3 round structure)', () => {
    const cup = initContinentalCup('南洲', TEAMS_8, 19, new SeededRNG(22));
    ...
    expect(cup.rounds[0].fixtures[0].id).toMatch(/^CC-southern_cup-S19-QF-M1$/);
  });

  it('builds a 东洲杯 QF round (tagged eastern_cup)', () => {
    const cup = initContinentalCup('东洲', TEAMS_8, 21, new SeededRNG(33));
    expect(cup.type).toBe('eastern_cup');
    expect(cup.region).toBe('东洲');
    expect(cup.rounds[0].fixtures[0].id).toMatch(/^CC-eastern_cup-S21-QF-M1$/);
  });

  it('produces deterministic pairings for the same seed', () => {
    const a = initContinentalCup('大陆', TEAMS_16, 1, new SeededRNG(42));
    const b = initContinentalCup('大陆', TEAMS_16, 1, new SeededRNG(42));
...
describe('getContinentalCupCurrentFixtures', () => {
  it('returns the active round fixtures', () => {
    const cup = initContinentalCup('大陆', TEAMS_16, 17, new SeededRNG(7));
...
describe('advanceContinentalCup', () => {
  it('runs 大陆杯 R16 → QF → SF → Final and marks completed after the Final', () => {
    let cup = initContinentalCup('大陆', TEAMS_16, 17, new SeededRNG(11));
    ...
    expect(cup.rounds[1].roundName).toBe('QF');
    expect(cup.rounds[1].fixtures).toHaveLength(4);
    expect(cup.completed).toBe(false);

    // QF → SF
    results = cup.rounds[1].fixtures.map(homeWins);
    cup = advanceContinentalCup(cup, results);
    expect(cup.rounds[2].roundName).toBe('SF');
    expect(cup.rounds[2].fixtures).toHaveLength(2);

    // SF → Final
    results = cup.rounds[2].fixtures.map(homeWins);
    cup = advanceContinentalCup(cup, results);
    expect(cup.rounds[3].roundName).toBe('Final');
    expect(cup.rounds[3].fixtures).toHaveLength(1);
    expect(cup.completed).toBe(false);

    // Final
    const finalFixture = cup.rounds[3].fixtures[0];
  ...
  it('runs 南洲杯 QF → SF → Final (3 rounds for 8-team cups)', () => {
    let cup = initContinentalCup('南洲', TEAMS_8, 19, new SeededRNG(99));
    // QF
    ...
    // SF
    cup = advanceContinentalCup(cup, cup.rounds[1].fixtures.map(homeWins));
    expect(cup.rounds[2].roundName).toBe('Final');
    expect(cup.rounds[2].fixtures).toHaveLength(1);
    // Final
    const finalFix = cup.rounds[2].fixtures[0];
  ...
  it('resolves a final tied after extra time via penalty shootout', () => {
    let cup = initContinentalCup('东洲', TEAMS_8, 21, new SeededRNG(123));
  ...
  it('throws when a fixture is missing from results', () => {
    const cup = initContinentalCup('大陆', TEAMS_16, 17, new SeededRNG(1));
    // Pass only half the results
    const partial = cup.rounds[0].fixtures.slice(0, 4).map(homeWins);
