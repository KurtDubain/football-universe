    ...
    <PageShell width="narrow">
      ...
      <div className="bg-[var(--surface-panel)] rounded-lg border border-slate-700 overflow-hidden">
        ...
        <div className="divide-y divide-slate-700/40">
          {SETTINGS_GUIDE_ITEMS.map((item) => {
            const expanded = guideOpen === item.key;
            const contentId = `settings-guide-${item.key}`;
            return (
              <div key={item.key}>
                <button
                  type="button"
                  ...
                  className="flex min-h-12 w-full cursor-pointer items-center justify-between px-4 py-2.5 text-left transition-colors hover:bg-slate-700/30"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center text-emerald-300">
                  ...
                  <Icon
                    name="arrow-down"
                    size={15}
                    className={`shrink-0 text-slate-500 transition-transform duration-200 motion-reduce:transition-none ${expanded ? 'rotate-180' : ''}`}
                  />
                </button>
                {expanded && (
                  <div id={contentId} className="px-4 pb-3 pl-[3.25rem]">
