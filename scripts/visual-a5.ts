...
async function main() {
  ...
  // Advance season part-way to get matches with stats (and trigger highlights)
  for (let i = 0; i < 25; i++) {
  ...
  // Snap dashboard to capture focus player section + fixture cards
  await page.screenshot({ path: '/tmp/a5-dashboard.png', fullPage: false });

  // Snap league page for best-scorer column
  await page.goto(URL + 'league/1', { waitUntil: 'networkidle' });
  await page.waitForTimeout(800);
  await page.screenshot({ path: '/tmp/a5-league.png', fullPage: false });
